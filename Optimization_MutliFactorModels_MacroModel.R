...
# строим многофакторную макроэкономическую модель
sigma.model <- function(R){
  
  macro.ret <- read.csv(file = "macro_returns.csv", header = T, stringsAsFactors = F, sep = ",")
  macro.ret$Date <- as.Date(macro.ret$Date)
  macro.ret <-  xts(macro.ret[,-1], order.by=macro.ret[,1])
  head(macro.ret)

  time.series <- merge.xts(returns, macro.ret)

  macro.fit <- fitTimeSeriesFactorModel(assets.names=colnames(time.series),
                                        factors.names=c("BRENT","USD.RUB","COMEX.GC"),
                                        data=time.series,fit.method="DLS", decay.factor = 0.81)

  
  N = 50
  sigma <- factorModelCovariance(macro.fit$beta[seq(1:N),],var(macro.ret),macro.fit$resid.variance[seq(1:N)])
