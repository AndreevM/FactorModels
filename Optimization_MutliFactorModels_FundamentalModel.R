...
# задаем фундаментальную модель
sigma.model <- function(R){

  stock.m <- read.csv(file = "industry_factors.csv", header = T, stringsAsFactors = F, sep = ",")

  # stock.m[c(1,730,3315, 3702),]

  exposure.names <- c("OIL","ENERGY","FINANCE","RETAIL", "OTHER")

  # строим многофакторную фундаментальную модель
  fundamental.fit <- fitFundamentalFactorModel(data=stock.m,
                                               exposure.names=exposure.names,
                                               datevar = "DATE",
                                               returnsvar = "RETURN",
                                               assetvar = "TICKER", wls = T,
                                               regression = "classic",
                                               covariance = "classic", full.resid.cov = F,
                                               robust.scale = F)
  # извлекаем ковариационную матрицу из модели
  sigma <- fundamental.fit$returns.cov$cov
...
# Мультифакторная фундаментальная модель
stock.m <- read.csv(file = "industry_factors.csv", header = T, stringsAsFactors = F, sep = ",")
...
fundamental.fit$resids.cov
fundamental.fit$factor.cov$cov
fundamental.fit$tstats
fundamental.fit$returns.cov$cov


# выводим на график доходности риск-факторов
library(PerformanceAnalytics)
chart.CumReturns(fundamental.fit$factor.returns, main = "Кумулятивные доходности риск-факторов", legend.loc="topleft")
